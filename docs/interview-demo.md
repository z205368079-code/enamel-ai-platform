...
Customer → Chatwoot → Gateway → MaxKB/RAG → Chatwoot; an explicit `NO_ANSWER` may use the optional DeepSeek general-guidance fallback. High risk always → HUMAN. Chatwoot owns conversations/agents; MaxKB owns knowledge retrieval/RAG; this project owns the Gateway, webhook/client adapters, state, idempotency, retry, migration, handoff, internal auth, analytics and knowledge gaps.
...
4. Ask an unknown company question: show the DeepSeek fallback if configured; disable it to show gap, handoff and internal stats.
...
- `NO_ANSWER` becomes a knowledge gap only if the optional DeepSeek fallback is unavailable or fails; a human handoff can also be user-requested or high-risk and is not always a gap.
